...
class SpeakerModelingLM(PreTrainedModel):
    ...
    def forward(
        ...
        start = start.to(model_inputs.device)
        middle = middle.to(model_inputs.device)
        end = end.to(model_inputs.device)
        labels_padded = torch.cat([start, text, middle, torch.tensor([-100], device=model_inputs.device, dtype=text.dtype).repeat(B, 1), codes_list, end], dim=1)
